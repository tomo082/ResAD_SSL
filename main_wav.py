...
from validate_wav import validate  # validate_wav をインポート
...
# ==========================================
# Haar Wavelet Filter (S2SWCLIP / WMOE 思想)
# ==========================================
class HaarWaveletFilter(nn.Module):
    def __init__(self, low_freq_weight=0.1, high_freq_weight=1.5):
        """
        低周波(LL: 位置ズレや全体構造)を抑制し、高周波(HL,LH,HH: キズやエッジ)を強調する
        """
        super().__init__()
        self.lf_w = low_freq_weight
        self.hf_w = high_freq_weight
        
        # Haarウェーブレットのカーネル (2x2)
...
def main(args):
    ...
    # 1. ウェーブレットフィルタの初期化 (低周波を0.1に抑え、高周波を1.2倍に強調)
    wav_filter = HaarWaveletFilter(low_freq_weight=0.1, high_freq_weight=1.2).to(args.device)
    
    # 2. constraintorのみ初期化 (vq_opsは削除)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            for class_name in CLASSES['unseen']:
                ...
                # validate_wav の呼び出し (wav_filter と constraintor を渡す)
